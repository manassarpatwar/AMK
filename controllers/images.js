...
exports.getAll = function (req, res) {
  try {
    Image.find({}, 'roomNo title author description url', function (err, images) {
      if (err)
        res.status(500).send('Invalid data!');
      res.setHeader('Content-Type', 'application/json');
...
exports.getByRoom = function (userData, res) {
  try {
    Image.find({roomNo: userData.roomNo}, 'roomNo title author description url', function (err, images) {
      if (err)
        res.status(500).send('Invalid data!');
      let image = []
      if (images.length >= 1)
...
exports.getById = function (userData, res) {
  try {
    Image.find({_id: userData._id}, 'roomNo title author description url', function (err, images) {
      if (err)
        res.status(500).send('Invalid data!');
